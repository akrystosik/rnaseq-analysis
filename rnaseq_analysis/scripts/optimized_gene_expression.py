...
# Then modify the load_expression_data function to preferentially load the combined dataset
def load_expression_data(data_dir: Union[str, Path] = DEFAULT_DATA_DIR, use_combined: bool = True) -> ExpressionDataLoader:
    """
    ...
    if use_combined and COMBINED_DATA_FILE.exists():
        loader.datasets['combined'] = ad.read_h5ad(COMBINED_DATA_FILE)
        loader.loaded_datasets.add('combined')
        loader._build_indices('combined', loader.datasets['combined'])
        logger.info("Loaded combined dataset with all data except GTEx")
    else:
...
def get_gene_expression_matrix(dataset: str,
    ...
    Args:
        dataset: Name of the dataset
        genes: Tuple of gene symbols or Ensembl IDs (use tuple for cache compatibility)
    ...
    # Convert tuples to lists for the function call
    genes_list = list(genes)
    donors_list = list(donors) if donors else None
    tissues_list = list(tissues) if tissues else None
    
    # Get expression matrix
    expr_matrix, gene_ids, sample_ids = data_loader.get_expression_matrix(
        dataset, genes_list, donors_list, tissues_list
    )
...
def get_tissue_gene_expression(dataset: str, gene: str, tissue: str, 
    ...
    """
    return get_gene_expression(dataset, gene, None, tissue, data_loader)
...
def get_donor_gene_expression(dataset: str, gene: str, donor: str, 
    ...
    Returns:
        Dictionary with expression statistics
    """
    return get_gene_expression(dataset, gene, donor, None, data_loader)
