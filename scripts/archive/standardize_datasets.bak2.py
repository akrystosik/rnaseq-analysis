...
from rnaseq_utils import (
    ...
    standardize_metadata, validate_metadata, prepare_for_anndata,
    load_mappings, load_json_mapping, load_dataset_specific_metadata,
...
def create_standard_anndata(data_df, obs_df, var_df, dataset_info):
    ...
    for key in dataset_info.keys():
        if isinstance(dataset_info[key], (pd.Series, pd.DataFrame, np.ndarray)):
            logger.debug(f"Converting uns[{key}] from {type(dataset_info[key])} to standard Python type")
            dataset_info[key] = dataset_info[key].tolist() if hasattr(dataset_info[key], 'tolist') else str(dataset_info[key])
    ...
    logger.debug(f"Creating AnnData with X shape={data_df.values.shape}, obs shape={validated_obs_df.shape}, var shape={var_df.shape}")
    adata = ad.AnnData(
        X=data_df.values,
        obs=validated_obs_df,
        var=var_df
    )
    ...
    if adata.var.shape[1] == 0:
        ...
        logger.error(f"Original var_df had shape {var_df.shape} with columns: {list(var_df.columns)}")
    ...
    if adata.obs.shape[1] == 0:
        ...
        logger.error(f"Original obs_df had shape {validated_obs_df.shape} with columns: {list(validated_obs_df.columns)}")
...
def save_anndata(adata, file_path):
    ...
    try:
        ...
        if adata.obs.index.name is not None and adata.obs.index.name in adata.obs.columns:
            logger.warning(f"Fixing index/column name conflict: Renaming column '{adata.obs.index.name}' to 'original_{adata.obs.index.name}'")
            adata.obs = adata.obs.rename(columns={adata.obs.index.name: f'original_{adata.obs.index.name}'})
            
        if adata.var.index.name is not None and adata.var.index.name in adata.var.columns:
            logger.warning(f"Fixing index/column name conflict: Renaming column '{adata.var.index.name}' to 'original_{adata.var.index.name}'")
            adata.var = adata.var.rename(columns={adata.var.index.name: f'original_{adata.var.index.name}'})
        ...
        logger.info(f"Verification: Saved AnnData loaded with var shape={test_load.var.shape}, obs shape={test_load.obs.shape}")
...
def load_entex_metadata(metadata_file=None):
    ...
    try:
        if metadata_file and os.path.exists(metadata_file):
            ...
            metadata_path = Path("/mnt/czi-sci-ai/intrinsic-variation-gene-ex/rnaseq/encode/metadata/entex_metadata.json")
        ...
        # Handle both formats - the old format with nested structure and the new flat format
        if isinstance(metadata, dict) and ('entex_metadata' in metadata or 'sample_lookup' in metadata):
            # Old format with nested structure
            sample_lookup = metadata.get('sample_lookup', {})
            if not sample_lookup and 'entex_metadata' in metadata:
            ...
            # New format - flat dictionary with file_id as keys
            sample_lookup = metadata
            metadata = {
                "sample_lookup": sample_lookup,
                "donor_map": {}  # We'll build this from the samples
            }
            
            # Create donor map
            donor_map = {}
            for file_id, info in sample_lookup.items():
                donor_id = info.get('donor', info.get('subject_id'))
        ...
        logger.info(f"Loaded metadata for {len(metadata['sample_lookup'])} ENTEx samples")
        return metadata
...
def process_encode_data(input_dir, entex_dir=None, output_file=None, entex_metadata_file=None, metadata_dir=None):
    """
    ...
    # Load ENTEx metadata
    entex_metadata = load_entex_metadata(entex_metadata_file) if (entex_dir or entex_metadata_file) else None
    ...
    if entex_dir:
        ...
        # Check metadata file
        if entex_metadata:
            logger.info(f"ENTEx metadata loaded with {len(entex_metadata.get('sample_lookup', {}))} samples")
        else:
            ...
            for group_key, files in candidate_groups.items():
                if len(files) == 1:
                    ...
                    if preferred_files:
                        ...
                        filtered_samples.append(files[0])
                        logger.debug(f"Group {group_key}: no file meets hg38; keeping first file: {files[0]}")

            logger.info(f"Selected {len(filtered_samples)} ENTEx gene quantification file(s) after grouping by donor and tissue")
            tsv_files.extend(filtered_samples)

        else:
            # If no metadata filtering is possible, apply basic filtering to file paths
            gene_files = [f for f in entex_files if 'gene' in f.lower() and 'transcript' not in f.lower()]
            logger.info(f"Selected {len(gene_files)} ENTEx files that appear to be gene quantifications")
...
def process_entex_data(metadata_file, output_file, metadata_dir=None):
    ...
    # Save the standardized AnnData
    if save_anndata(adata, output_file):
        logger.info(f"Successfully processed ENTEx data with {adata.n_obs} samples and {adata.n_vars} genes")
...
def process_gtex_data(input_file, output_file, metadata_dir=None):
    ...
    if 'sample_id' in obs_df.columns:
        logger.info("Renaming 'sample_id' column to 'original_sample_id' to avoid conflict with index")
...
# Add ENTEx-specific arguments
    ...
    parser.add_argument('--dataset-type', choices=['encode', 'entex', 'gtex', 'mage', 'adni'], 
...
def main():
    ...
    if args.entex_metadata_file and args.dataset_type == 'entex':
        ...
        if entex_data is not None:
            logger.info(f"Successfully processed ENTEx data: {entex_data.n_obs} samples and {entex_data.n_vars} genes")
    ...
    if processed_datasets:
        ...
        for name, data in processed_datasets:
            if 'metadata_validation' in data.uns:
                ...
                logger.info(f"  Missing fields: {', '.join(report['missing_fields']) if report['missing_fields'] else 'None'}")
