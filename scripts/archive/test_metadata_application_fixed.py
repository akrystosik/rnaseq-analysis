#!/usr/bin/env python3
"""
Test applying metadata from JSON files to AnnData objects - with type conversion fix
"""
import scanpy as sc
import pandas as pd
import numpy as np
...
def convert_to_serializable(obj):
    """Convert all values in a nested object to serializable types."""
...
def apply_metadata(adata, metadata):
    ...
    if 'dataset_info' in metadata:
        # Update uns with dataset info - convert to serializable types
        for key, value in metadata['dataset_info'].items():
            serialized_value = convert_to_serializable(value)
            adata.uns[key] = serialized_value
    ...
    # Add sample_id field which was missing from the first test
    adata.obs['sample_id'] = adata.obs.index
    logger.info(f"Added obs[sample_id] using index values")
    ...
    for dataset_name in ['adni', 'encode', 'gtex', 'mage']:
        ...
        if metadata:
            ...
            if 'dataset_info' in metadata:
                ...
                # Convert metadata to serializable types
                for key, value in metadata['dataset_info'].items():
                    metadata['dataset_info'][key] = convert_to_serializable(value)
                logger.info("Converted metadata to serializable types")
            else:
            ...
            try:
                ...
                # Now load it back to verify
                loaded_adata = sc.read_h5ad(test_file)
                logger.info(f"Successfully loaded {dataset_name} test AnnData back from file")
                logger.info(f"Loaded dataset has {len(loaded_adata.obs.columns)} obs columns and {len(loaded_adata.var.columns)} var columns")
            
            except Exception as e:
                logger.error(f"Error saving or loading {dataset_name} AnnData: {e}")
                import traceback
