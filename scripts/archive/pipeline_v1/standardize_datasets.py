...
def ensure_serializable(obj):
    ...
    if obj is None:
        ...
        try:
            # Attempt to convert to Python native type if possible
            item_value = obj.item()
            # Check if the resulted item is still a complex numpy type
            if isinstance(item_value, (np.integer, np.floating)):
                return item_value.item() # Try one more level
            elif isinstance(item_value, np.bool_):
                 return bool(item_value)
            elif pd.isna(item_value):
                 return None # Represent NaN as None
            return item_value # Return native type if successful
        except Exception as e:
            logger.debug(f"Could not convert numpy scalar {obj} of type {type(obj)} to native type: {e}. Using str().")
            return str(obj) # Fallback to string
    elif isinstance(obj, dict):
        # Ensure keys are strings and values are serializable
        return {str(k): ensure_serializable(v) for k, v in obj.items()}
    elif isinstance(obj, (list, tuple, set)):
        # Recursively serialize elements in lists/tuples/sets
        return [ensure_serializable(x) for x in obj]
    elif hasattr(obj, 'tolist'):  # Handle numpy arrays
        try:
            return obj.tolist()
        except Exception as e:
            logger.debug(f"Could not convert numpy array-like object {type(obj)} to list: {e}. Using str().")
            return str(obj) # Fallback
    elif hasattr(obj, 'to_dict'):  # Handle pandas Series/DataFrame (use with caution for large DFs)
        try:
            # Simple dict conversion for Series, might need more nuance for DataFrames
             if isinstance(obj, pd.Series):
                 return obj.astype(str).to_dict() # Convert to string first
             elif isinstance(obj, pd.DataFrame):
                 logger.warning(f"Converting DataFrame in uns to dict. This might be large.")
                 return obj.astype(str).to_dict() # Convert to string first
             else:
                return obj.to_dict()
        except Exception as e:
            logger.debug(f"Could not convert pandas object {type(obj)} to dict: {e}. Using str().")
            return str(obj) # Fallback
    elif pd.isna(obj):
         return None # Represent pandas NA/NaN as None
    else:
        # Last resort: convert to string if not recognized
        return str(obj)
...
def save_anndata(adata, file_path):
    """Save AnnData object to file with validation and serialization."""
    try:
        ...
        if adata.n_vars == 0:
            ...
            return False
        if adata.var.shape[1] == 0:
             logger.error("Cannot save AnnData: var DataFrame has no columns!")
             # Attempt to reconstruct if index seems okay
             if len(adata.var_names) > 0:
                 logger.warning("Attempting to reconstruct var DataFrame from index.")
        ...
        # Ensure all `uns` values are serializable before saving
        ...
        # Restore original uns in the in-memory object after saving
        adata.uns = original_uns

        # Optional: Verify successful save by trying to load it back
        ...
        logger.error(f"Error saving AnnData to {file_path}: {e}")
        import traceback
        logger.error(traceback.format_exc())
        # Attempt to restore original uns even on error
        if 'original_uns' in locals():
             adata.uns = original_uns
