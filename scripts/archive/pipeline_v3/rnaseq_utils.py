    ...
    if 'metadata_source' in metadata:
        if 'metadata_sources' not in adata.uns:
            adata.uns['metadata_sources'] = []
             ...
             # If it exists but isn't a list, make it one
             adata.uns['metadata_sources'] = [adata.uns['metadata_sources']]

        # --- Modification: Append as JSON string ---
        try:
             # Ensure the metadata_source dict itself is serializable first
             # (though ensure_serializable on the whole uns later should catch this)
             source_str = json.dumps(metadata['metadata_source'])
             adata.uns['metadata_sources'].append(source_str)
        except Exception as e:
             logger.error(f"Could not serialize metadata_source to JSON: {e}. Appending as string.")
             adata.uns['metadata_sources'].append(str(metadata['metadata_source']))
        # --- End Modification ---
