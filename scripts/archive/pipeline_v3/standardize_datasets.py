...
def ensure_serializable(obj):
    ...
    # Handle numpy scalars first (most specific)
    if isinstance(obj, np.integer): return int(obj)
    if isinstance(obj, np.floating): return None if np.isnan(obj) else float(obj)
    if isinstance(obj, np.bool_): return bool(obj)
    if isinstance(obj, np.void): return None # Or appropriate representation

    # Handle pandas scalars / NA types
    if pd.isna(obj): return None
    if isinstance(obj, pd.Timestamp): return obj.isoformat()

    # Handle numpy arrays (check size AFTER basic types/scalars)
    if isinstance(obj, np.ndarray):
        if obj.size == 0: return []
        if obj.dtype == 'object':
             # If array contains objects, recursively serialize elements
             ...
             if np.issubdtype(obj.dtype, np.floating):
                 ...
                 return obj.tolist() # Assume other basic dtypes convert cleanly

    # Handle simple Iterables (list, tuple, set)
    if isinstance(obj, (list, tuple, set)):
        # Check for list of dictionaries specifically
        temp_list = list(obj)
        if temp_list and all(isinstance(item, dict) for item in temp_list):
            logger.debug("List of dicts found, converting elements and dumping to JSON string.")
            try:
                # Ensure inner dicts are fully serializable first
                serializable_list = [ensure_serializable(item) for item in temp_list]
                # Convert the list of *serializable* dicts to a JSON string
                return json.dumps(serializable_list)
            except Exception as e:
                logger.warning(f"Could not serialize list of dicts to JSON: {e}. Falling back to string repr.")
    ...
    if isinstance(obj, dict):
        # Must serialize keys to strings and values recursively
