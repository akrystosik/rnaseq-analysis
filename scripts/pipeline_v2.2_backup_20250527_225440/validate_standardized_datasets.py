...
def validate_dataset(file_path):
    ...
    try:
        ...
        for field, config in metadata_fields.items():
            if field in adata.obs.columns:
                ...
                if 'ontology_field' in config and config['ontology_field'] in adata.obs.columns:
                    ...
                    # Handle multiple ontology prefixes
                    if 'ontology_prefixes' in config:
                        ontology_prefixes = config['ontology_prefixes']
                        # Count values with any of the correct prefixes
                        valid_values = pd.Series([False] * adata.n_obs, index=adata.obs.index)
                        for prefix in ontology_prefixes:
                            valid_values |= adata.obs[ontology_field].astype(str).str.startswith(prefix)
                        valid_count = valid_values.sum()
                    else:
                        # Single prefix (backward compatibility)
        ...
        if adata.n_vars > 0:
            # Check gene ID formats more comprehensively
            all_genes = adata.var_names.astype(str)
            ...
            other_count = len(all_genes) - ensembl_count - entrez_count - spike_in_count
            
            total_genes = len(all_genes)
            ensembl_percentage = (ensembl_count / total_genes) * 100 if total_genes > 0 else 0
            
            # Determine format based on majority and acceptable standards
            ...
            validation_results['validations']['gene_id_format'] = {
                ...
                'details': {
                    ...
                    'entrez_count': entrez_count,
                    'spike_in_count': spike_in_count,
                    'other_count': other_count,
                    'ensembl_percentage': round(ensembl_percentage, 1)
                }
            }
            
            if format_status == 'failed' and validation_results['status'] == 'passed':
                validation_results['status'] = 'warning'
