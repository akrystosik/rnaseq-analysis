...
def main():
    ...
    # --- 4.5. Apply Cell Type Ontology Mapping ---
    logger.info("Applying cell type ontology mapping...")
    celltype_mapping_file = os.path.join(args.metadata_json_dir, "celltype_to_cl.json")
    if os.path.exists(celltype_mapping_file) and 'cell_type' in adata_pb.obs.columns:
        try:
            import json
            with open(celltype_mapping_file, 'r') as f:
                celltype_data = json.load(f)
                celltype_mappings = celltype_data.get('mappings', {})
            ...
            for idx, row in adata_pb.obs.iterrows():
                ...
                if cell_type in celltype_mappings:
                    ...
                    if isinstance(ontology_info, dict) and not cell_type.startswith('_comment'):
            ...
            logger.info(f"Cell type ontology mapping complete: {mapped_count}/{adata_pb.n_obs} samples mapped to CL terms")
        except Exception as e:
            logger.warning(f"Error applying cell type ontology mapping: {e}")
            # Initialize empty columns as fallback
            if 'cell_type_ontology_term_id' not in adata_pb.obs.columns:
                adata_pb.obs['cell_type_ontology_term_id'] = ''
            if 'cell_type_ontology_confidence' not in adata_pb.obs.columns:
                adata_pb.obs['cell_type_ontology_confidence'] = 'none'
    else:
        logger.warning(f"Cell type mapping file not found at {celltype_mapping_file} or 'cell_type' column missing. Skipping cell type ontology mapping.")
